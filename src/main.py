...
game_state = MENU          # Startzustand = Hauptmenü
player_name = ""
score_saved = False
level = 1
final_score = 0
...
# -----------------------
def draw_ui_banner():
    """Zeichnet die obere UI-Leiste mit Timer, Level, Spielername, Dash-Status und Gold."""
    pygame.draw.rect(screen, (30, 30, 30), (0, 0, WIDTH, UI_HEIGHT))
    pygame.draw.line(screen, (200, 200, 200), (0, UI_HEIGHT), (WIDTH, UI_HEIGHT), 2)

    # Timer
    time_text = small_font.render(f"Time: {timer.remaining()}s", True, (255, 255, 255))
    ...
    # Dash-Kreis und Spielername
    dash_radius, dash_x_offset = 10, 20
    dash_y = UI_HEIGHT // 2
    ...
    screen.blit(name_text, (name_x, UI_HEIGHT // 2 - name_text.get_height() // 2))
...
def draw_centered_text(text, font, color, y_offset=0):
    """Zeichnet Text zentriert auf dem Bildschirm."""
...
def draw_win_screen():
    """Anzeige, wenn das Level gewonnen wurde."""
...
def draw_game_over_screen():
    """Anzeige, wenn der Spieler gestorben ist."""
...
def draw_leaderboard(y_start=40):
    """Zeigt die Bestenliste an (Top 5 nach Score)."""
    scores = load_scores()
    scores.sort(key=lambda e: e["Score"], reverse=True)  # Sortierung nach Score absteigend
    ...
    for i, entry in enumerate(scores[:5]):
        ...
        screen.blit(line, (WIDTH // 2 - line.get_width() // 2, HEIGHT // 2 + y_start + 30 + i * 30))
...
def draw_leaderboard_screen():
    """Zeigt die gesamte Bestenliste an."""
...
def main():
    ...
    while running:
        ...
        if game_state == MENU:
            ...
            if keys[pygame.K_SPACE]: player.dash(walls)
            if dx != 0 and dy != 0: dx *= 0.7; dy *= 0.7
...
# -----------------------
def restart_game():
    """Setzt das Level, Spieler, Gegner, Gold und Timer zurück."""
    global maze, walls, player, enemies, timer, game_state, exit_rect, golds, gold_count, final_score
    ...
    # Gegnerliste erstellen (Anzahl = Level)
    enemies.clear()
    ...
    # Gold an zufälligen freien Zellen platzieren
    for _ in range(level):
        if not free_cells:
            break
        px, py = random.choice(free_cells)
        free_cells.remove((px, py))
        golds.append(Gold(px + CELL_SIZE // 2, py + CELL_SIZE // 2))
